...
//ini_set('memory_limit', '-1');
    ...
    date_default_timezone_set('America/Caracas');
	  ...
	  //107.161.189.226
	  //138.128.182.50
  $this->conexion = @mysqli_connect("108.179.194.12","felcason_demo","DeMoLiDeR");
// $this->conexion = @mysqli_connect("localhost","root","");


   if($this->conexion && mysqli_select_db( $this->conexion,"felcason_demoliderplus"))
   {	   
    return true;
   }
   else  
			 ...
			 $cursor   = $bd->ejecutar_sql($sql);
			  ...
			  if ($row = $bd->proximafila($cursor))
			  {
					DO
					{
						...
						$filas [$contador]["mobservaci"] = $config->sanear_string(trim($row["mobservaci"]));				
						...
						$cantidad2=mysqli_num_rows($cursor2);
	  ...
	  if ($row = $bd->proximafila($cursor))
	  {
			DO
			{
				...
				if($row["nmonto_h"]<>'') {
				    ...
				    $filas [$contador]["nmonto_h"] = '0.00';
				}
				if($row["nmonto_d"]<>'') {
				    $filas [$contador]["nmonto_d"] = $row["nmonto_d"];
				}
				else{
				    $filas [$contador]["nmonto_d"] = '0.00';
				}
