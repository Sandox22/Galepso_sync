...
USE SHARED (dirdata +  ;
    "tcontroles.dbf") IN 0
SET DATABASE TO c:\galepso\data\emp3\bdemp.dbc
...
IF lnhandle > 0
     ...
     SELECT tproductos_almacen
     SET FILTER TO .NOT. DELETED().AND. ALLTRIM(cid_almace) = "03"
      GO bottom
     ...
     DO WHILE  .NOT. EOF()     
		    ...
		    ('<<ALLTRIM(tproductos_almacen.cid_produc)>>','<<'3'>>','<<Str(tproductos_almacen.nexisten,5,2)>>','<<Str(tproductos_almacen.ncosto_inv,5,2)>>','<<Str(tproductos_almacen.ncosto_ulc,5,2)>>')
			ENDTEXT	
     ...
     CLOSE DATABASES all
       **===============TRANSFIRIENDO EXISTENCIAS POR ALMACEN==================
		lcSQLcommand=lcSQLcommand+";"		
		lBandera=.f.
		SQLEXEC(lnhandle,"Delete from tproductos_almacen where TRIM(tproductos_almacen.cid_almace)='3'")			
		SQLEXEC(lnhandle,lcSQLcommand)
		SQLEXEC(lnhandle, "UPDATE tsincronizacion set dact_existen=CONVERT_TZ(NOW(),'-4:30','-8:00') where cid_empre='3'")
